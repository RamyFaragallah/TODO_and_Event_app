...
abstract class _$Database extends GeneratedDatabase {
  ...
  Future<int> _update(
    ...
    return (operateOn ?? this).customUpdate(
      'UPDATE todo SET is_finish = ? WHERE id = ?',
      variables: [
        Variable.withBool(var1),
        Variable.withInt(var2),
      ],
      updates: {todo},
    );
  }
  ...
  Future<List<TodoData>> _getByDay(
      DateTime var1,
      int var2,
      {@Deprecated('No longer needed with Moor 1.6 - see the changelog for details')
          QueryEngine operateOn}) {
    return (operateOn ?? this).customSelect(
        'SELECT * FROM todo WHERE time=? AND todo_type = ? ',
        variables: [
          Variable.withDateTime(var1),
          Variable.withInt(var2),
        ]).then((rows) => rows.map(_rowToTodoData).toList());
  }

  Stream<List<TodoData>> watchGetByDay(DateTime var1, int var2) {
    return customSelectStream(
        'SELECT * FROM todo WHERE time=? AND todo_type = ? ',
        variables: [
          Variable.withDateTime(var1),
